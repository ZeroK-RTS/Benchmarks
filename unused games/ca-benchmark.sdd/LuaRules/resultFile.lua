...
function output:WriteTest(test,fpsTable)
  ...
  if ((#fpsTable) > 0) then
    self:WriteLine("!transmitlobby "..test.name.." min. FPS: " .. round(minFPS,0))
    self:WriteLine("!transmitlobby "..test.name.." avg. FPS: " .. round(avgFPS/(#fpsTable),0))
    self:WriteLine("!transmitlobby "..test.name.." max. FPS: " .. round(maxFPS,0))
  end
  self.file:flush()
end
...
function output:WriteTestRuntime(test,runtime,ideal)
  self.file:write("-----[ ".. (test.name) .." ]-------------------------------------------------------\n")
  self:WriteLine("!transmitlobby ".. test.name.. " runtime: " .. math.floor((runtime/ideal)*100) .. "% (" .. runtime .. "s/" .. ideal .. "s)")
  self.file:flush()
end
